...
export const apiService = {
  ...
  async generateApiKey(keyName: string): Promise<ApiKeyResponse> {
    const response = await api.post<ApiKeyResponse>(`/auth/api-key/generate`, null, {
  ...
  async getSystemLogs(
    ...
    const response = await api.get<LogsResponse>(`/system/logs/${encodeURIComponent(service)}`, {
      params: { lines },
    })
    return response.data
  },
  ...
  async getVMessUsers(configFile?: string): Promise<VMessUsersResponse> {
    ...
    const response = await api.get<VMessUsersResponse>('/vmess/users', { params })
    return response.data
  },
