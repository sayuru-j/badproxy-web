...
export const ErrorMessage: React.FC<ErrorMessageProps> = ({
  ...
  const variants = {
    error: {
      ...
      icon: AlertCircle,
      iconColor: effectiveTheme === "dark" ? "text-red-400" : "text-red-600",
    },
    warning: {
      styles:
        effectiveTheme === "dark"
  ...
  const buttonHoverStyle =
    effectiveTheme === "dark" ? "hover:bg-white/10" : "hover:bg-black/10";
  ...
  if (compact) {
    return (
      <div
        ...
        ${currentSize.container} ${currentVariant.styles} 
      ...
      >
        ...
        <div className="flex flex-col space-y-2 sm:hidden">
          <div className="flex items-start space-x-2">
            <IconComponent
              className={`${currentSize.icon} mt-0.5 flex-shrink-0 ${currentVariant.iconColor}`}
            />
          ...
          {(onRetry || onDismiss) && (
            <div className="flex items-center justify-end space-x-2 pt-1">
              {onRetry && (
                <button
                  onClick={onRetry}
                  className={`${currentSize.button} ${buttonHoverStyle} rounded transition-colors flex items-center space-x-1`}
                  title="Retry"
                >
        ...
        <div className="hidden sm:flex items-start space-x-3">
          ...
          />
          <div className="flex-1 min-w-0">
            {title && <h4 className={`${currentSize.title} mb-1`}>{title}</h4>}
            <p className={`${currentSize.text}`}>{message}</p>
          </div>
          {(onRetry || onDismiss) && (
            <div className="flex items-center space-x-2">
              {onRetry && (
                <button
                  onClick={onRetry}
                  className={`${currentSize.button} ${buttonHoverStyle} rounded transition-colors`}
                  title="Retry"
                >
                  <RefreshCw className={currentSize.buttonIcon} />
                </button>
        ...
        {(onRetry || onDismiss) && (
          <div className="flex flex-col sm:flex-row items-end sm:items-center space-y-1 sm:space-y-0 sm:space-x-2 flex-shrink-0 ml-2">
            {onRetry && (
              <button
                onClick={onRetry}
                className={`${currentSize.button} ${buttonHoverStyle} rounded transition-colors flex items-center space-x-1 sm:space-x-0`}
                title="Retry"
              >
                <RefreshCw className={currentSize.buttonIcon} />
